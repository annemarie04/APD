...
def jacobi_ring(A, b, x0, max_iter=100, tol=1e-6):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    if rank == 0:
        print(f"Starting Ring Jacobi: Solving {n} equations on {size} processes")
...
def main():
    ...
    # Solve using Ring Jacobi
    x, iterations, exec_time = jacobi_ring(A, b, x0, max_iter=100, tol=1e-6)
